/**
 * JWD File Picker
 * @Copyright Fadjri Custom
 * @website https://fadjri.com
 * @relesase 2021-05-31
 * @update 2023-06-28
 */
...
(function ($) {
    ...
    this.jwdfilepicker.show_alert = function (icon, title, message) {
        Swal.fire({
            title: title,
            text: message,
            icon: icon,
    ...
    this.jwdfilepicker.init = function (options = {}) {
        ...
        if (options.server_url == "" || options.icon_url == "") {
            var message =
                "jwdfilepicker: server_url and icon_url must not empty !!!";
            jwdfilepicker.show_alert("error", "Error !!", message);
            // console.log(options);
            return;
        }
        console.log(options.id_file);
        ...
        var html_error_data_notfound =
            '<div class="alert alert-danger error-data-notfound">Data tidak ditemukan</div>';
        ...
        var $modal_container = $(
            '<div class="wdi-modal-content ' + options.margin_style + '">'
        ).appendTo($modal);
        var $modal_header = $('<div class="wdi-modal-header">').appendTo(
            $modal_container
        );
        var $modal_body = $(
            '<div class="wdi-modal-body">' +
        ...
        // FOOTER
        var $modal_footer = $('<div class="wdi-modal-footer"></div>').appendTo(
            $modal_container
        );
        if (options.id_file == "") {
            text = "Pilih File";
            ...
            $(
                '<button class="btn btn-primary btn-choose-file disabled" disabled>' +
                    text +
                    "</button>"
        ...
        $(
            '<button class="btn btn-secondary nav-mobile-file-properties disabled" disabled>File Properties <i class="fas fa-chevron-up arrow-up"></i></button>'
        ).appendTo($modal_footer);
        ...
        html_header +=
            ...
            '<button class="nav-link active" id="file-browser-tab" data-bs-toggle="tab" data-bs-target="#file-browser" type="button" role="tab" aria-controls="file browser" aria-selected="false">' +
            tab_title +
            "</button>" +
        ...
        $dropzone = $(
            '<form action="' +
                options.server_url +
                '/ajaxUploadFile" class="dropzone" id="file-dropzone">' +
                '<input type="hidden" name="_token" value="' +
                csrf +
                '">' + // Add this line
                '<div class="dz-message dz-default needsclick">' +
                '<div><i class="fas fa-cloud-upload-alt text-gray"></i></div>' +
        ...
        // File browser
        var $container = $(
            '<div class="jwd-file-picker-container row"></div>'
        ).appendTo($modal_body.find("#file-browser"));
        var $file_container = $(
            '<div class="col-xs-12 col-md-7 col-lg-9 left-panel"></div>'
        ).appendTo($container);
        var $meta_container = $(
            '<div class="col-xs-12 col-md-5 col-lg-3 right-panel">' +
        ...
        $.getJSON(
            ...
            function (data) {
                ...
                html =
                    '<div class="loading-status text-light bg-success shadow-sm" style="display:none">Showing: <span class="loading-item">' +
                    data.loaded_item +
                    '</span> / <span class="total-item">' +
                    data.total_item +
                    "</span></div>" +
                    '<div class="left-panel-menu form-inline">' +
                    '<select class="form-select filter-file" name="filter">';
                ...
                html +=
                    form_options +
                    "</select>" +
                    '<select class="form-select filter-tgl" name="Tanggal"><option value="" selected>Semua Tanggal</option>';
                ...
                for (k in item) {
                    // console.log(item[k]['id'])
                    src = item[k]["thumbnail"]["url"];

                    var img_class = " file-thumbnail";
                    if (
                        item[k]["file_type"] == "image" &&
                        item[k]["file_not_found"] == "false"
                    ) {
                        var img_class = "";
                        if (options.id_file) {
                            var src = item[k]["url"];
                    ...
                    html +=
                        '<li class="jwd-thumbnail-item" id="file-' +
                        item[k]["id"] +
                        '" ' +
                        initial_item +
                        ">" +
                        ...
                        src +
                        '">' +
                        '<span class="meta-file" style="display:none">' +
                ...
                // For Scrolling
                loading_item = parseInt(
                    $modal_body.find(".loading-item").html()
                );
        ...
        $(".jwd-btn-copy").click(function () {
            ...
            var $text_success = $(
                '<span class="text-copied text-success ms-2">Link copied...</span>'
            ).insertAfter($this);
            setTimeout(function () {
        ...
        $("body").delegate(".jwd-thumbnail-item", "click", function () {
            ...
            $meta_wrapper
                .find(".btn-delete-file")
                .attr(
                    "data-delete-title",
                    "Hapus permanen file <strong>" +
                        meta.nama_file +
                        "</strong> ?"
                )
            ...
            if (meta_file.default !== undefined) {
                if (meta_file.default.width !== undefined) {
                    $meta_wrapper
                        ...
                        .html(
                            "w: " +
                                meta_file.default.width +
                                " x h: " +
                                meta_file.default.height
                        );
                }
            }
            ...
            for (k in meta.file_exists.thumbnail) {
                if (meta.file_exists.thumbnail[k] == "not_found") {
                    file_not_found +=
                        "<li>" + meta_file.thumbnail[k]["filename"] + "</li>";
                }
            }
            ...
            if (file_not_found) {
                $(".file-exists")
                    .empty()
                    .show()
                    .append("File not found: <ul>" + file_not_found + "</ul>");
            } else {
        ...
        $(".jwd-alt-text, .jwd-title, .jwd-description, .jwd-caption").blur(
            function () {
                    ...
                    $.ajax({
                        ...
                        success: function (message) {
                            ...
                            if (message.status == "ok") {
                                ...
                                $(".list-file-container")
                                    .find("#file-" + meta_json.id)
                                    .find(".meta-file")
                                    .html(JSON.stringify(meta_json));

                                var $result = $(
                                    '<span class="result-badge text-light bg-success">Saved</span>'
                                )
                                    .hide()
                                    .insertAfter($this);
                                ...
                                $(
                                    '<span class="result-badge bg-danger">Error...</span>'
                                ).insertAfter($this);
                            }
                        },
                        error: function () {},
                ...
                callback: function (confirmed) {
                    if (confirmed) {
                        $.ajax({
                            ...
                            data:
                                "_method=delete&submit=submit&ajax=ajax&id=" +
                                $this.attr("data-id"),
                                        ...
                                        function () {
                                            if (
                                                ...
                                                $modal_body
                                                    .find(".left-panel")
                                                    .append(
                                                        html_error_data_notfound
                                                    );
                                                $btn_choose_file.attr(
                                                    "disabled",
                                                    "disabled"
                                                );
                                            } else {
        ...
        $(".btn-choose-file")
            .off("click")
            .on("click", function () {
                $elm = $(".list-file-container")
                    .find(".list-highlighted")
                    .find(".img-container");
        ...
        $("body").delegate("#myTab button, #myTab a", "click", function () {
            ...
            if (id == "upload-tab") {
                ...
                $highlighted = $(".list-file-container").find(
                    ".list-highlighted"
                );
        ...
        var $wdi_modal_footer = $(".wdi-modal-footer");
        var $arrow_up = $wdi_modal_footer.find(".arrow-up");
        $("body").on("click", ".nav-mobile-file-properties", function () {
            var z_index = $right_panel.css("z-index");
            if (z_index === undefined || z_index == 0) {
                $right_panel.css("z-index", 1).fadeIn("fast", function () {
                    $arrow_up.addClass("rotated");
                });
            } else {
                $right_panel.fadeOut("fast", function () {
                    $arrow_up.removeClass("rotated");
                    $right_panel.css("z-index", 0).show();
                });
            }
        });
		...
		*/
        ...
        document.addEventListener(
            ...
            function (event) {
                ...
                if ((scrollHeight >= scrollPos) / scrollHeight == 0) {
                    // console.log('bottom-scroll');
                    var $loading_status = $(".loading-status"); // Define and assign $loading_status variable
                    if ($loading_status.is(":visible")) {
                        return;
                    }
                    if (loading_item < total_item) {
        ...
        $("body").delegate(".search-files", "keyup search", function () {
            ...
            timer = setTimeout(function () {
                ...
                $btn_choose_file.removeAttr("disabled");
                $loader = $('<div class="loader-ring">').appendTo(
                    $file_container
                );
                $(".meta-file-wrapper").hide();
                page = 0;
                ...
                $.getJSON(
                    ...
                    function (data) {
                        ...
                        if (data.data.length == 0) {
                            ...
                            $(html_error_data_notfound).appendTo(
                                $(".thumb-container")
                            );
                            return;
                        }
        ...
        $("body").delegate(".filter-tgl, .filter-file ", "change", function () {
            ...
            page = 0;
            $ul.empty();
            $.getJSON(options.server_url + "?" + url_filter(), function (data) {
                $loader.remove();
                total_item = data.total_item;
                loading_item = data.loaded_item;
                if (data.data.length == 0) {
        ...
        function load_file(data) {
            item = data[0];
            // console.log(item.id_file_picker);
            $li_new = $li.clone().hide();
            $li_new.attr("id", "file-" + item.id);
            ...
            $img.attr("src", item.thumbnail.url)
                .on("load", function () {})
                .on("error", function (e) {
                    console.log(e);
                    $this = $(this);
                    url_alternative_image =
                        options.icon_url + "file_not_found.png";
		...
		*/
        ...
        var jwdDropzone = new Dropzone(target, {
            ...
            clickable: true,
            dictFallbackMessage:
                "Browser Anda tidak support drag'n'drop file uploads.",
            dictFileTooBig:
                "Ukuran file Anda terlalu besar: ({{filesize}}MiB). Ukuran maksimal file yang diperkenankan: {{maxFilesize}}MiB.",
        ...
        jwdDropzone.on("success", function (file, response) {
            ...
            let meta_file = JSON.parse(response.file_info.meta_file);
            ...
            $li.find(".filename").html(response.file_info["nama_file"]);
            $li.attr("id", "file-" + response.file_info["id"]);
            $img_cont
                .find("img")
                .attr("src", response.file_info["thumbnail"]["url"]);
            $img_cont
                .find(".meta-file")
                .html(JSON.stringify(response.file_info));

            $img_cont.find("img").removeClass("file-thumbnail");
            if (response.file_info["file_type"] != "image") {
            ...
            if (options.filter_file != "") {
                filter_match = false;
                split = options.filter_file.split(" ");
                if (split.indexOf(response.file_info["file_type"]) != -1) {
                    filter_match = true;
                }
            }

            if (filter_match) {
                for (k in response.file_info["bulan_upload"]) {
                    if (!(k in list_filter_tgl)) {
                        $(
                            '<option value="' +
                                k +
                                '">' +
                                response.file_info["bulan_upload"][k] +
                                "</option>"
                        ).insertAfter(
                            $modal_body.find(".filter-tgl").children().eq(0)
                        );
                        list_filter_tgl[k] =
                            response.file_info["bulan_upload"][k];
                    }
